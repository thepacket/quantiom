...
export function QSpherePanel({ state }: Props) {
  ...
  const markers = useMemo(() => {
    ...
    // Group basis indices by Hamming weight; assign an angular position within each ring.
    ...
    for (let w = 0; w <= n; w++) {
      ...
      const theta = (Math.PI * w) / n; // 0 = top (|0...0⟩), π = bottom
      const m = ring.length;
      for (let k = 0; k < m; k++) {
        const idx = ring[k];
  ...
  if (!data) return null;
  const symbolic = data.amplitudes.some((a) => a.re === null);
  const cx = SIZE / 2;
  const cy = SIZE / 2;

  return (
    <section className="panel panel--qsphere">
      ...
      {symbolic ? (
        ...
        <svg width={SIZE} height={SIZE} className="qsphere__svg">
          ...
          {/* markers — drawn back-to-front for depth */}
          {[...markers]
            .sort((a, b) => b.y - a.y)
            .map((m) => {
              ...
              if (m.mag < 1e-6) {
                return (
                  <circle key={m.idx} cx={p.sx} cy={p.sy} r={1.5} className="qsphere__zero" />
                );
              }
                ...
                <g key={m.idx}>
                  ...
                  <circle cx={p.sx} cy={p.sy} r={radius} fill={color} className="qsphere__marker">
                    <title>
                      ...
                      arg = {(Math.atan2(m.im, m.re) * (180 / Math.PI)).toFixed(1)}°
                    </title>
                  </circle>
                </g>
              );
