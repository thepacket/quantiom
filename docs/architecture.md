...
sim/structureFactor.ts  S(k) — FT of the connected ⟨ZⱼZₗ⟩ correlator
...
- **Framework: Vitest** (`client/test/*.test.ts`), run in Node. **1011
...
- **Continuous integration.** `.github/workflows/ci.yml` type-checks
  ...
  on **every push and pull request**. A green build means all 1011 tests
  passed.
